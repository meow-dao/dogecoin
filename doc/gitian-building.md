...
*Setup instructions for a Gitian build of Meow Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Meow
...
- [Building Meow Core](#building-meow-core)
...
Clone the git repositories for meow and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/meow/meow.git
git clone https://github.com/meow/gitian.sigs.git
```
...
in the meow repository under 'Fetch and create inputs' to install sources which require
...
To build Meow Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the meow repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/meow/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/meow/meow
...
For example:
```bash
URL=https://github.com/laanwj/meow.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit meow=${COMMIT} --url meow=${URL} ../meow/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit meow=${COMMIT} --url meow=${URL} ../meow/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit meow=${COMMIT} --url meow=${URL} ../meow/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the meow git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../meow/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/meow/meow-detached-sigs.git

BTCPATH=/some/root/path/meow
SIGPATH=/some/root/path/meow-detached-sigs

./bin/gbuild --url meow=${BTCPATH},signature=${SIGPATH} ../meow/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/meow-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/meow-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/meow-osx-build.assert
```
...
[meow/gitian.sigs](https://github.com/meow/gitian.sigs/) repository, or if that's not possible create a pull
